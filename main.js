	...
	document.addEventListener('DOMContentLoaded', function() {
		if (document.getElementById('calcular')) {
			...
			if (formulario_editar.length > 0) {
				if (pase_día.value || pase_dosdías.value || pase_completo) {
					mostrarDias();
				}
			}
			...
			pase_día.addEventListener('input', mostrarDias);
			pase_dosdías.addEventListener('input', mostrarDias);
		...
		if (document.getElementById('mapa')) {
			...
			L.marker([-32.979256, -60.642235]).addTo(map).bindPopup('GDLWebCAMP 2024').openPopup();
